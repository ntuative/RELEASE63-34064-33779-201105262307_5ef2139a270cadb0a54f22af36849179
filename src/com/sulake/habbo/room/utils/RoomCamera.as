package com.sulake.habbo.room.utils
{
   import com.sulake.room.utils.IVector3d;
   import com.sulake.room.utils.Vector3d;
   
   public class RoomCamera
   {
      
      private static const const_1616:Number = 12;
       
      
      private var var_2892:int = -1;
      
      private var var_2889:int = -2;
      
      private var var_216:Vector3d = null;
      
      private var var_1327:Number = 0;
      
      private var var_1607:Number = 0;
      
      private var var_2069:Boolean = false;
      
      private var var_196:Vector3d = null;
      
      private var var_2071:Vector3d;
      
      private var var_2893:Boolean = false;
      
      private var var_2884:Boolean = false;
      
      private var var_2886:Boolean = false;
      
      private var var_2888:Boolean = false;
      
      private var var_2887:int = 0;
      
      private var var_2890:int = 0;
      
      private var _scale:int = 0;
      
      private var var_2885:int = 0;
      
      private var var_2891:int = 0;
      
      private var var_1779:int = -1;
      
      private var var_2070:int = 0;
      
      private var var_2068:Boolean = false;
      
      public function RoomCamera()
      {
         this.var_2071 = new Vector3d();
         super();
      }
      
      public function get location() : IVector3d
      {
         return this.var_196;
      }
      
      public function get targetId() : int
      {
         return this.var_2892;
      }
      
      public function get targetCategory() : int
      {
         return this.var_2889;
      }
      
      public function get targetObjectLoc() : IVector3d
      {
         return this.var_2071;
      }
      
      public function get limitedLocationX() : Boolean
      {
         return this.var_2893;
      }
      
      public function get limitedLocationY() : Boolean
      {
         return this.var_2884;
      }
      
      public function get centeredLocX() : Boolean
      {
         return this.var_2886;
      }
      
      public function get centeredLocY() : Boolean
      {
         return this.var_2888;
      }
      
      public function get screenWd() : int
      {
         return this.var_2887;
      }
      
      public function get screenHt() : int
      {
         return this.var_2890;
      }
      
      public function get scale() : int
      {
         return this._scale;
      }
      
      public function get roomWd() : int
      {
         return this.var_2885;
      }
      
      public function get roomHt() : int
      {
         return this.var_2891;
      }
      
      public function get geometryUpdateId() : int
      {
         return this.var_1779;
      }
      
      public function get isMoving() : Boolean
      {
         if(this.var_216 != null && this.var_196 != null)
         {
            return true;
         }
         return false;
      }
      
      public function set targetId(param1:int) : void
      {
         this.var_2892 = param1;
      }
      
      public function set targetObjectLoc(param1:IVector3d) : void
      {
         this.var_2071.assign(param1);
      }
      
      public function set targetCategory(param1:int) : void
      {
         this.var_2889 = param1;
      }
      
      public function set limitedLocationX(param1:Boolean) : void
      {
         this.var_2893 = param1;
      }
      
      public function set limitedLocationY(param1:Boolean) : void
      {
         this.var_2884 = param1;
      }
      
      public function set centeredLocX(param1:Boolean) : void
      {
         this.var_2886 = param1;
      }
      
      public function set centeredLocY(param1:Boolean) : void
      {
         this.var_2888 = param1;
      }
      
      public function set screenWd(param1:int) : void
      {
         this.var_2887 = param1;
      }
      
      public function set screenHt(param1:int) : void
      {
         this.var_2890 = param1;
      }
      
      public function set scale(param1:int) : void
      {
         if(this._scale != param1)
         {
            this._scale = param1;
            this.var_2068 = true;
         }
      }
      
      public function set roomWd(param1:int) : void
      {
         this.var_2885 = param1;
      }
      
      public function set roomHt(param1:int) : void
      {
         this.var_2891 = param1;
      }
      
      public function set geometryUpdateId(param1:int) : void
      {
         this.var_1779 = param1;
      }
      
      public function set target(param1:IVector3d) : void
      {
         var _loc2_:* = null;
         if(this.var_216 == null)
         {
            this.var_216 = new Vector3d();
         }
         if(this.var_216.x != param1.x || this.var_216.y != param1.y || this.var_216.z != param1.z)
         {
            this.var_216.assign(param1);
            this.var_2070 = 0;
            _loc2_ = Vector3d.dif(this.var_216,this.var_196);
            this.var_1327 = _loc2_.length;
            this.var_2069 = true;
         }
      }
      
      public function dispose() : void
      {
         this.var_216 = null;
         this.var_196 = null;
      }
      
      public function initializeLocation(param1:IVector3d) : void
      {
         if(this.var_196 != null)
         {
            return;
         }
         this.var_196 = new Vector3d();
         this.var_196.assign(param1);
      }
      
      public function resetLocation(param1:IVector3d) : void
      {
         if(this.var_196 == null)
         {
            this.var_196 = new Vector3d();
         }
         this.var_196.assign(param1);
      }
      
      public function update(param1:uint, param2:Number) : void
      {
         var _loc3_:* = null;
         var _loc4_:Number = NaN;
         var _loc5_:Number = NaN;
         var _loc6_:Number = NaN;
         var _loc7_:Number = NaN;
         if(this.var_216 != null && this.var_196 != null)
         {
            ++this.var_2070;
            if(this.var_2068)
            {
               this.var_2068 = false;
               this.var_196 = this.var_216;
               this.var_216 = null;
               return;
            }
            _loc3_ = Vector3d.dif(this.var_216,this.var_196);
            if(_loc3_.length > this.var_1327)
            {
               this.var_1327 = _loc3_.length;
            }
            if(_loc3_.length <= param2)
            {
               this.var_196 = this.var_216;
               this.var_216 = null;
               this.var_1607 = 0;
            }
            else
            {
               _loc4_ = Math.sin(0 * _loc3_.length / this.var_1327);
               _loc5_ = param2 * 0.5;
               _loc6_ = this.var_1327 / const_1616;
               _loc7_ = _loc5_ + (_loc6_ - _loc5_) * _loc4_;
               if(this.var_2069)
               {
                  if(_loc7_ < this.var_1607)
                  {
                     _loc7_ = this.var_1607;
                     if(_loc7_ > _loc3_.length)
                     {
                        _loc7_ = _loc3_.length;
                     }
                  }
                  else
                  {
                     this.var_2069 = false;
                  }
               }
               this.var_1607 = _loc7_;
               _loc3_.div(_loc3_.length);
               _loc3_.mul(_loc7_);
               this.var_196 = Vector3d.sum(this.var_196,_loc3_);
            }
         }
      }
      
      public function reset() : void
      {
         this.var_1779 = -1;
      }
   }
}
