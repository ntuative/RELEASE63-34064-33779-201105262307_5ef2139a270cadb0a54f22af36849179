package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_750:ColorTransform;
      
      private var var_1435:ColorTransform;
      
      private var var_1759:Matrix;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_750 = new ColorTransform();
         this.var_750.alphaMultiplier = 1;
         this.var_1435 = new ColorTransform();
         this.var_1759 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_103 = this.var_1156[param1];
         this.var_949 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1760 >= 0)
         {
            this.var_255 = this._visualization.getSprite(this.var_1760);
            if(this.var_255 && this.var_255.asset)
            {
               ...
               if(this.var_88 == null)
               {
                  this.var_88 = this.var_255.asset.clone();
                  if(this.var_750.alphaMultiplier != 1)
                  {
                     this.var_609 = new BitmapData(this.var_88.width,this.var_88.height,true,4278190080);
                  }
               }
               this.var_2443 = -this.var_255.offsetX;
               this.var_2441 = -this.var_255.offsetY;
               this.var_255.asset = this.var_88;
            }
            if(this.var_88)
            {
            ...
            if(this.var_609)
            {
               this.var_609.fillRect(this.var_609.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_103.y * this.var_1155;
         }
         return 0;
      }
      ...
      {
         if(!this.var_103 || !this.var_255)
         {
            return;
         }
         if(this.var_88 && this.var_255.asset != this.var_88)
         {
            this.var_255.asset = this.var_88;
         }
         if(this.var_949)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_949 && this.var_103.hasIgnited)
         {
            this.var_949 = true;
         }
         _loc8_ = this.var_1114 * this.var_1155;
         this.var_103.update();
         if(this.var_949)
         {
            if(this.var_103.roomObjectSpriteId >= 0)
            ...
            {
               this.var_88.draw(this.var_609,this.var_1759,this.var_750,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_103.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2443 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1155;
               _loc4_ = this.var_2441 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1155;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1435.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_88.draw(_loc7_,this._translationMatrix,this.var_1435,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2442 = parseInt(param1.@size);
         this.var_1760 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1114 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1155 = this.var_2442 / 64;
         this._blend = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this._blend = Math.min(this._blend,1);
         this.var_750.alphaMultiplier = this._blend;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc19_ = _loc23_.@is_emitter == "ToggleToRandomState" ? false : true;
