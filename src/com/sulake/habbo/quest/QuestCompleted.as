package com.sulake.habbo.quest
{
   ...
   public class QuestCompleted implements IDisposable
   {
      ...
      private function onNextQuest(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this._window.visible = false;
         if(this.var_304.completedQuestsInCampaign >= this.var_304.questCountInCampaign)
         {
            this.var_304.id = 0;
            this.var_304.accepted = false;
            this._questEngine.questController.campaignCompleted.show(this.var_304);
         }
         else
         {
      ...
      public function prepare(param1:QuestMessageData) : void
      {
         ...
         this._window.findChildByName("catalog_link_txt").caption = this._questEngine.localization.getKey("quests.completed.cataloglink." + this.var_304.activityPointType);
         var _loc2_:String = "quests.completed.reward." + this.var_304.activityPointType;
         this._questEngine.localization.registerParameter(_loc2_,"amount",this.var_304.rewardCurrencyAmount.toString());
         this._window.findChildByName("reward_txt").caption = this._questEngine.localization.getKey(_loc2_,_loc2_);
         var _loc3_:* = this.var_304.getQuestLocalizationKey() + ".completed";
      ...
      private function onCatalogLink(param1:WindowEvent, param2:IWindow = null) : void
      {
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            this._questEngine.openCatalog(this.var_304);
         }
      }
