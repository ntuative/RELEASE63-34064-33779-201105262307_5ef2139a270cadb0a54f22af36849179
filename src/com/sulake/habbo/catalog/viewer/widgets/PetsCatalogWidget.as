package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(this.var_531 < 0 || this.var_531 > this.var_370.length)
         {
            this.var_531 = 0;
         }
         this.updateImage();
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_209 == null || this.var_791 >= this.var_209.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_531 >= 0 && this.var_531 < this.var_370.length)
         {
            _loc7_ = this.var_370[this.var_531] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_209[this.var_791] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_514,_loc3_,_loc2_,AvatarScaleType.const_57,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_348,AvatarAction.const_897);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_531 >= this.var_370.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_370[this.var_531] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_209[this.var_791] as int;
