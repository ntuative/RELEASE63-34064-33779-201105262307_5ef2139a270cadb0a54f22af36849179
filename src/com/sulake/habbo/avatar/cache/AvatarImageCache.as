package com.sulake.habbo.avatar.cache
{
   ...
   public class AvatarImageCache
   {
      ...
      private static const const_1958:int = 60000;
      ...
      private var var_350:IActiveActionData;
      ...
      private var var_2343:ByteArray;
      
      private var var_2341:ImageData;
      ...
      public function dispose() : void
      {
         ...
         {
            _loc2_ = this.var_251.getKeys();
            for each(_loc3_ in _loc2_)
            {
               _loc1_ = this.var_251.getValue(_loc3_) as AvatarImageBodyPartCache;
               if(_loc1_ != null)
               {
                  _loc1_.dispose();
               }
      ...
      public function disposeInactiveActions(param1:int = 60000) : void
      {
         ...
         var _loc4_:Array = this.var_251.getKeys();
         for each(_loc5_ in _loc4_)
         {
            _loc3_ = this.var_251.getValue(_loc5_) as AvatarImageBodyPartCache;
            if(_loc3_ != null)
            {
      ...
      public function resetBodyPartCache(param1:IActiveActionData) : void
      {
         var _loc2_:* = null;
         var _loc4_:* = null;
         var _loc3_:Array = this.var_251.getKeys();
         for each(_loc4_ in _loc3_)
         {
            _loc2_ = this.var_251.getValue(_loc4_) as AvatarImageBodyPartCache;
            if(_loc2_ != null)
            {
      ...
      public function setGeometryType(param1:String) : void
      {
         ...
         if(this.var_740 == GeometryType.const_1136 && param1 == GeometryType.const_1189 || this.var_740 == GeometryType.const_1189 && param1 == GeometryType.const_1136)
         {
            this.var_740 = param1;
            this.var_88 = null;
            return;
      ...
      public function getImageContainer(param1:String, param2:int) : AvatarImageBodyPartContainer
      {
         ...
         if(this._bodyPartCache == null)
         {
            ...
            this.var_251.add(param1,this._bodyPartCache);
         }
         var _loc3_:int = this._bodyPartCache.getDirection();
         var _loc4_:int = param2;
         this.var_350 = this._bodyPartCache.getAction();
         var _loc5_:IActiveActionData = this.var_350;
         var _loc6_:* = [];
         var _loc7_:Dictionary = new Dictionary();
         var _loc8_:Point = new Point();
         if(!(!this.var_350 || !this.var_350.definition))
         {
            if(this.var_350.definition.isAnimation)
            {
               _loc11_ = _loc3_;
               _loc12_ = this._structure.getAnimation(this.var_350.definition.state + "." + this.var_350.actionParameter);
               _loc13_ = param2 - this.var_350.startFrame;
               if(_loc12_ != null)
               {
                  _loc14_ = _loc12_.getLayerData(_loc13_,param1,this.var_350.overridingAction);
                  if(_loc14_ != null)
                  {
                     ...
                     {
                        this.var_350 = _loc14_.action;
                     }
                     if(_loc14_.type == AnimationLayerData.const_922)
                     {
                        if(_loc14_.action != null)
                        {
                     ...
                     else if(_loc14_.type == AnimationLayerData.const_636)
                     {
                        _loc3_ = _loc11_;
                     }
         ...
         if(_loc10_ == null)
         {
            _loc15_ = this._structure.getParts(param1,this.var_1393.getFigure(),_loc5_,this.var_740,_loc3_,_loc6_,_loc7_);
         ...
         if(this.var_473 == null)
         {
            ...
            this.var_473 = this.renderBodyPart(_loc3_,_loc16_,_loc4_,this.var_350);
            if(this.var_473 == null)
            {
               return null;
            }
      ...
      {
         var _loc2_:AvatarImageBodyPartCache = this.var_251.getValue(param1) as AvatarImageBodyPartCache;
         if(_loc2_ == null)
         {
            _loc2_ = new AvatarImageBodyPartCache();
            this.var_251.add(param1,_loc2_);
         }
         return _loc2_;
      }
      ...
      private function renderBodyPart(param1:int, param2:Array, param3:int, param4:IActiveActionData) : AvatarImageBodyPartContainer
      {
         ...
         if(!this.var_88)
         {
            this.var_88 = this._structure.getCanvas(this._scale,this.var_740);
            if(!this.var_88)
            {
               return null;
            }
            this.var_2342 = new Point(0,this.var_88.height - int(this.var_88.width / 4));
         }
         var _loc5_:int = param1;
         var _loc6_:Boolean = AvatarDirectionAngle.const_1707[param1];
         var _loc9_:String = param4.definition.assetPartDefinition;
         var _loc10_:String = param4.definition.state;
         var _loc11_:Boolean = _loc10_ == AvatarAction.const_957 || _loc10_ == AvatarAction.const_389;
         ...
         while(_loc8_ >= 0)
         {
            ...
            {
               this.var_1407 = this.var_117.bodyPartId;
               this.var_182 = this.var_117.partType;
               this.var_1719 = this.var_117.partId;
               ...
               if(_loc6_)
               {
                  ...
                  else
                  {
                     ...
                     if(this.var_117.flippedPartType != this.var_182)
                     {
                        this.var_182 = this.var_117.flippedPartType;
                     }
                  }
               }
               this.var_1406 = this._scale + "_" + this.var_117.action.assetPartDefinition + "_" + this.var_182 + "_" + this.var_1719 + "_" + _loc5_ + "_" + this._frame;
               this.var_424 = this._assets.getAssetByName(this.var_1406) as BitmapDataAsset;
               if(this.var_424 == null)
               {
                  this.var_1406 = this._scale + "_std_" + this.var_182 + "_" + this.var_1719 + "_" + _loc5_ + "_0";
                  this.var_424 = this._assets.getAssetByName(this.var_1406) as BitmapDataAsset;
               }
               if(!(!this.var_424 || !this.var_424.content))
               {
                  if(this.var_117.paletteMapId > -1)
                  {
                     ...
                     {
                        this.var_921 = this.var_424.content as BitmapData;
                        this.var_181 = new BitmapData(this.var_921.width,this.var_921.height);
                        this.var_2343 = this.var_934.colorize(this.var_921.getPixels(this.var_921.rect));
                        this.var_181.setPixels(this.var_181.rect,this.var_2343);
                     }
                     else
                     {
                        this.var_181 = (this.var_424.content as BitmapData).clone();
                     }
                  }
                  else
                  {
                  ...
                  if(_loc11_ && (this.var_1407 == AvatarBodyPartType.const_269 || this.var_1407 == AvatarBodyPartType.const_1657 || this.var_1407 == AvatarBodyPartType.const_1782) && this.var_117.color != null)
                  {
                     _loc16_ = 0.7;
                  ...
                  {
                     this.var_181.colorTransform(this.var_181.rect,this.var_117.color.colorTransform);
                  }
                  if(this.var_117.isBlendable)
                  {
                     this.var_181.colorTransform(this.var_181.rect,this.var_117.blendTransform);
                  }
                  this.var_922 = this.var_424.offset.clone();
                  if(_loc7_)
                  {
                     if(this._scale == AvatarScaleType.const_57)
                     {
                  ...
                  this.var_2341 = new ImageData(this.var_181,this.var_922,_loc7_);
                  this.var_920.push(this.var_2341);
               }
            }
            _loc8_--;
         }
         ...
         var _loc13_:ImageData = this.createUnionImage(this.var_920,_loc6_);
         var _loc14_:Point = new Point(-1 * _loc13_.regpoint.x,this.var_2342.y - _loc13_.regpoint.y);
         if(_loc6_)
         {
            if(this._scale == AvatarScaleType.const_57)
            {
         ...
         _loc8_ = this.var_920.length - 1;
         while(_loc8_ >= 0)
         {
            _loc15_ = this.var_920.pop();
            if(_loc15_)
            {
               _loc15_.dispose();
            }
