package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1949:uint = 2;
      
      protected static const const_1950:uint = 4;
      
      protected static const const_1947:uint = 3;
      
      protected static const name_4:uint = 7;
      
      public static const const_1761:int = 5;
      
      public static const LIBRARY_LOADER_FINALIZE:String = "LIBRARY_LOADER_FINALIZE";
      
      public static const const_1268:Boolean = false;
      ...
      protected var var_3024:uint = 0;
      
      protected var var_2951:Boolean = true;
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1268)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_796[_loc2_] as LibraryLoader;
               if(_loc1_ && (_loc1_.ready || _loc1_.disposed))
               {
                  var_796.splice(_loc2_,1);
               }
               _loc2_--;
            }
            while(var_796.length < const_1807 && false)
            {
               _loc1_ = var_1004.shift() as LibraryLoader;
      ...
      {
         return !!this.var_1103 ? uint(this.var_1363 - this.var_2172) : uint(getTimer() - this.var_2172);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_409,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_43.close();
            }
      ...
      {
         if(this.var_1102 && !_disposed)
         {
            this.var_1102 = false;
            if(!this.var_1103 && this.var_245)
            {
               this.var_1363 = -1;
               this.var_2172 = getTimer();
               queue(this);
            }
         }
      }
      ...
      {
         if(!this.var_1103 && !_disposed && this.var_1362 > 0)
         {
            try
            {
               this.var_43.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_245,const_1761 - this.var_1362);
            if(this.var_2951)
            {
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               ...
               this.debug("Load event STATUS " + this.var_409 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_409,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_409,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_409))
               {
         ...
         if(this.var_487 == LibraryLoader.const_1947)
         {
         ...
         if(this.var_487 == LibraryLoader.name_4)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_409,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_604 = this.getDefinition(this._name) as Class;
         ...
         try
         {
            xmlClass = this.var_604.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_409,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_409,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
