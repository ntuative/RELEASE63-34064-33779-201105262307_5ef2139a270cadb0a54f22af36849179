package com.sulake.core.window.services
{
   ...
   public class GestureAgentService implements IGestureAgentService, IDisposable
   {
      ...
      public function begin(param1:IWindow, param2:Function, param3:uint, param4:int, param5:int) : IWindow
      {
         ...
         {
            this._window = param1;
            this._window.addEventListener(WindowEvent.const_356,this.clientWindowDestroyed);
            this.var_287 = param2;
            this._working = true;
            this.var_1364 = param4;
            this.var_1365 = param5;
            this.var_201 = new Timer(40,0);
            this.var_201.addEventListener(TimerEvent.TIMER,this.operate);
            this.var_201.start();
         }
         return _loc6_;
      }
      ...
      protected function operate(param1:TimerEvent) : void
      {
         this.var_1364 *= 0.75;
         this.var_1365 *= 0.75;
         if(Math.abs(this.var_1364) <= 1 && Math.abs(this.var_1365) <= 1)
         {
            this.end(this._window);
         }
         else if(this.var_287 != null)
         {
            this.var_287(this.var_1364,this.var_1365);
         }
      }
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         {
            this.var_201.stop();
            this.var_201.removeEventListener(TimerEvent.TIMER,this.operate);
            this.var_201 = null;
         }
         if(this._working)
         {
            if(this._window == param1)
            {
               ...
               {
                  this._window.removeEventListener(WindowEvent.const_356,this.clientWindowDestroyed);
               }
               this._window = null;
               this._working = false;
            }
